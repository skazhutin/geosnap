...
def run(manifest_path: Path, errors_log: Path, retries: int) -> None:
    ...
    with errors_log.open("a", encoding="utf-8") as errors_fp:
        for _, row in df.iterrows():
            ...
            success = download_file(str(url), image_path, retries=retries, session=session)
            if success:
                ok += 1
            else:
                failed += 1
...
def main() -> None:
    ...
    run(manifest_path=Path(args.manifest), errors_log=Path(args.errors_log), retries=args.retries)
