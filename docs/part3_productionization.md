...
The bot implements `/start`, `/help`, highest-useful-resolution photo selection, pre/post-download byte limits, in-memory forwarding, request-ID propagation, per-user cooldown and bounded concurrency. It maps `ok` to evidence wording, coordinates, map link, attribution and native location; its score is explicitly not a probability. `low_confidence` and `out_of_coverage` never send a location pin. Backend rejection/rate/not-ready/timeout/malformed-response and Telegram failures are user-safe. It stores no photo, message, username or persistent Telegram identifier.
...
Frontend changes were limited to same-origin `/api`, development proxying, environment-driven production tiles/attribution, safe product-status handling, accurate confidence language, privacy/limitation copy and production build compatibility. Playwright verified the current desktop site, real upload/result/map and real abstention through Caddy with no console errors. This is functional production routing, not a final visual design; deferred work is in `docs/frontend_handoff.md`.

The final local gate passed Ruff, 422 Python tests (one explicit network-heavy opt-in skip), seven frontend tests, production build, Compose/Caddy validation, clean backend/bot builds, artifact reuse/integrity, real-index inference, restart, concurrency/rate/upload tests, browser smokes, bot integration/configuration smoke and the end-to-end production verifier. npm and bot dependency audits reported no known vulnerabilities. Backend auditing reported no known issues among identified pinned packages but could not map the direct CPU Torch/TorchVision wheel URLs to advisory package identities; those wheel hashes remain pinned and this limitation is operationally documented.
