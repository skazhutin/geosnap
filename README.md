...
- по завершении выполняет teardown через `docker compose down -v`, то есть не только останавливает контейнеры, но и удаляет контейнеры/сеть проекта и связанные тома; если вы используете тот же Compose project для других сервисов или данных, запускайте скрипт с осторожностью.
