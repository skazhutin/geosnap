...
GeoSnap is an experimental visual-geolocation product for supported Moscow street scenes. A user sends a street photo from the existing website or Telegram bot; one shared FastAPI service retrieves frozen street-view references and either returns an approximate location or abstains when the evidence is insufficient.
...
The development Vite server proxies `/api` to `http://localhost:8000`. Development may use the documented OpenStreetMap fallback; production builds require explicit tile URL and attribution. Research, ingestion, embedding, and historical evaluation targets remain in the `Makefile` but are not part of deployment.
...
- [Frontend phase handoff](docs/frontend_handoff.md)
...
The next product phase is a dedicated frontend redesign. The current site is production-routable and functional, but it is intentionally not described as the final visual experience.
